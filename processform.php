...
include('includes/injection.php');
	...
	if(isset($_POST['Register_User'])){
		...
		if(mysqli_num_rows($check1)==0){
			registration($fullname,$gender,$mobile,$email,$recorder,$lname,$username1,$position,$connect);
			}
		elseif(mysqli_num_rows($check2)==0){
			registration($fullname,$gender,$mobile,$email,$recorder,$lname,$username2,$position, $connect);
			}
		elseif(mysqli_num_rows($check3)==0){
			registration($fullname,$gender,$mobile,$email,$recorder,$lname,$username3,$position, $connect);
			}
		elseif(mysqli_num_rows($check4)==0){
			registration($fullname,$gender,$mobile,$email,$recorder,$lname,$username4,$position, $connect);
			}
		else{
	...
	function registration($fullname,$gender,$mobile,$email,$recorder,$lname,$username,$position, $connect){
		$url = 'corpus.php?manageUser=1';
		
		mysqli_query($connect, "INSERT INTO users (fullname, gender, mobileno, email, recorder, recorddate, lastupdate) VALUES('$fullname','$gender','$mobile','$email','$recorder',now(),now())");
		if(mysqli_error($connect)){
			$error = urlencode('<span style="color:red"><br/>User registration failed.'.mysqli_error($connect).'</span>');
			$url .= "&error=$error";
			echo '<meta http-equiv="refresh" content="0; url='.$url.'" />';
			exit;
			}
	...
	if(isset($_POST['Add_Category'])){
		...
		// for the parent category if null, then it's the parent
		$parent = ($parent=="")? "NULL":"'$parent'";	
		mysqli_query($connect, "INSERT INTO category (parentid, title) VALUES ($parent, '$category')");
		if(mysqli_error($connect)){
	...
	if(isset($_POST['Upload_Corpus'])){
		...
		for($i=0; $i<sizeof($content); $i++){
			...
			foreach($line as $row){
				...
				foreach($words as $word){
					$str .= (strlen($word)<1)? '':strtolower($word).' ';
					}
				}
			}
		$content = trim($str);
		...
		$corpus = $content;
			...
			// else{
				...
				$corpus = mysqli_query($connect, "INSERT INTO corpus (id, categoryid, author, title, pub_info, content, recorder, lastupdate, pub_year) 
				                                          VALUES(DEFAULT,'$category','$author', '$title', '$pub_info', '$corpus', '$username',now(),'$year')");
				if($corpus) {
                    $error = urlencode("<span style='color:blue'>Upload was successful<br/></span>");
				} else {
					var_dump(mysqli_error_list($connect));
					exit("Heloo");
					$error = urlencode("<span style='color:blue'>Upload failed<br/></span>");
				}
				
				$url = "corpus.php?uploadCorpus=1&message=$error";
				echo '<meta http-equiv="refresh" content="0; url='.$url.'">';
				exit;
				// }
		...
		///	$qcorpus = mysqli_fetch_assoc($qcorpus);
			//$recorder = $qcorpus['recorder'];
			//$corpus = $qcorpus['content'];
	...
	if(isset($_POST['Download'])){
		...
		//query contents
		$get_corpus = mysqli_query($connect, "SELECT * FROM corpus $WHERE");
		...
		$corpus_array = array();
		$corpus_str = '';
	while($content = mysqli_fetch_assoc($get_corpus)){
		$line = $content['content'];
		$title = $content['title'];
		$corpus_str .= $title."\n";
		$corpus_str .= $line."\n";
		$corpus_str .= strtoupper($content['author'])."\n\n";
		...
		// exit();
		// var_dump($line);
		// while(list($content) = mysqli_fetch_array($get_corpus, MYSQLI_ASSOC)){
		...
		// exit($corpus_str);
		$category = mysqli_query($connect, "SELECT title from category WHERE categoryid='$category'");
		$category = mysqli_fetch_assoc($category)['title'];

		
		//download the file
		$filename = "corpus_".$category.".txt";
...
EOD;
	...
	if(isset($_POST['Edit_User'])){
		...
		// exit(gettype($connect));
		//print msg
		$msg = (mysqli_error($connect))? urlencode("<span style='color:red'>User update failed'.mysqli_error($connect).'</span>"):urlencode("<span style='color:blue'>User was updated</span>");
